...
export default function BlogsPage() {
  ...
  const { data: blogsResponse } = useQuery(
    getBlogsQuery(locale, {
      ...
      categoryId: searchParams.blogCategoryId,
      limit: BLOGS_LIMIT * searchParams.page,
    }),
  );
  const { data: categoriesResponse } = useQuery(getBlogCategoriesQuery(locale));

  const blogs = blogsResponse?.result?.data ?? [];
  const pagination = blogsResponse?.result?.pagination ?? null;
  ...
  return (
    <PageTransition>
      <main>
        <PageHeader subtitle={t("BlogSubTitle")} title={t("Blog")} />
                ...
                {categories.map((category) => (
                  <motion.button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                      searchParams.blogCategoryId === category.id
                        ? "bg-primary text-white dark:bg-blue-700"
                        : "bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                    }`}
                    onClick={() => {
                      setSearchParams({ blogCategoryId: category.id, page: 1 });
                    }}
                  >
                    {category.name}
                  </motion.button>
            ...
            {blogs.length > 0 ? (
              <>
                {blogs.slice(0, 1).map((item) => (
                  ...
                  >
                    <Link href={`/blogs/${item.id}`} prefetch={true} aria-label={item.title} className="group relative overflow-hidden rounded-lg lg:col-span-3">
        ...
        <SectionWrapper className="bg-gray-50 dark:bg-gray-900">
          <div className="container mx-auto px-4 md:px-10">
            <SectionTitle subtitle="BLOG" title={t("Blog")} />
            {blogs.length > 1 ? (
              <>
