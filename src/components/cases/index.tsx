...
export default function CasesPage() {
  ...
  const { data: casesData } = useQuery(
    getCasesQuery(locale, {
      ...
      categoryId: searchParams.caseCategoryId,
      limit: CASES_LIMIT * searchParams.page,
    }),
  );
  const { data: categoriesData } = useQuery(getCaseCategoriesQuery(locale));

  const cases = casesData?.result?.data ?? [];
  const pagination = casesData?.result?.pagination ?? null;
  ...
  return (
    <PageTransition>
      <main>
        <PageHeader subtitle={t("CaseSubTitle")} title={t("Case")} />
                ...
                {categories.map((category) => (
                  <motion.button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                      searchParams.caseCategoryId === category.id
                        ? "bg-primary text-white dark:bg-blue-700"
                        : "bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                    }`}
                    onClick={() => {
                      setSearchParams({ caseCategoryId: category.id, page: 1 });
                    }}
                  >
                    {category.name}
                  </motion.button>
            ...
            {cases.length > 0 ? (
              <>
                {cases.slice(0, 1).map((item) => (
                  ...
                  >
                    <Link href={`/cases/${item.id}`} prefetch={true} aria-label={item.title} className="group relative overflow-hidden rounded-lg lg:col-span-3">
        ...
        <SectionWrapper className="bg-gray-50 dark:bg-gray-900">
          <div className="container mx-auto px-4 md:px-10">
            <SectionTitle subtitle="CASE" title={t("Case")} />
            {cases.length > 1 ? (
              <>
