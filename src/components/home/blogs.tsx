...
import { BlogCard } from "@/components/blogs/blogs-card";
...
export function Blogs() {
  ...
  /** 取得資料 */
  const { data: blogsData } = useQuery(getBlogsQuery(locale, { categoryId }));
  const { data: categoriesData } = useQuery(getBlogCategoriesQuery(locale));

  const blogs = blogsData?.result?.data ?? [];
  const categories = (() => {
        ...
        <div className="mb-10">
          ...
          >
            <div className="mb-2 flex items-center justify-center gap-2">
              ...
              <h2 className="text-sm font-medium text-primary">TECH BLOG</h2>
              <span className="block h-0.5 w-5 bg-primary"></span>
            </div>
            <AnimatedText
              as="h2"
