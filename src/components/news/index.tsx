...
export default function NewsPage() {
  ...
  const { data: newsResponse } = useQuery(
    getNewsQuery(locale, {
      ...
      categoryId: searchParams.newsCategoryId,
      limit: NEWS_LIMIT * searchParams.page,
    }),
  );
  const { data: categoriesResponse } = useQuery(getNewsCategoriesQuery(locale));

  const news = newsResponse?.result?.data ?? [];
  const pagination = newsResponse?.result?.pagination ?? null;
  ...
  return (
    <PageTransition>
      <main>
        <PageHeader subtitle={t("NewsSubTitle")} title={t("News")} />
                ...
                {categories.map((category) => (
                  <motion.button
                    ...
                    className={`rounded-full px-4 py-2 text-sm font-medium transition-colors ${
                      searchParams.newsCategoryId === category.id
                        ? "bg-primary text-white dark:bg-blue-700"
                        : "bg-white text-gray-700 hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-300 dark:hover:bg-gray-700"
                    }`}
                    onClick={() => {
                      setSearchParams({ newsCategoryId: category.id, page: 1 });
                    }}
                  >
                    {category.name}
                  </motion.button>
            ...
            {news.length > 0 ? (
              <>
                {news.slice(0, 1).map((item) => (
                  ...
                  >
                    <Link href={`/news/${item.id}`} prefetch={true} aria-label={item.title} className="group relative overflow-hidden rounded-lg lg:col-span-3">
        ...
        <SectionWrapper className="bg-gray-50 dark:bg-gray-900">
          <div className="container mx-auto px-4 md:px-10">
            <SectionTitle subtitle="NEWS" title={t("News")} />
            {news.length > 1 ? (
              <>
